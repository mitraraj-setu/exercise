...
for i in mylist1:
    ...
    if warehouse not in x:
       x[warehouse] = []
    x[warehouse].append({"product": i["product"], 'quantity': i['quantity']})
print(x)
...
for i in mylist1:
    ...
    if product not in X[warehouse]:
        X[warehouse][product] = 0

    X[warehouse][product] += quantity

X = {warehouse: [
    {'product': product, 'Quantity': quantity}
    for product, quantity in products.items()]
        for warehouse, products in X.items()}
print(X)
