...
my = [{'product': 'furn_0789', 'quantity': 16.0, 'warehouse': 'san francisco'},
...
my = sorted(my, key=lambda x: (x['product'], x['warehouse']))
for key, value in groupby(my, key=itemgetter("product","warehouse")):
    qty = 0
    final_product = False
    wa = False
    for v in value:
        # print(v)
        wa = v.get('warehouse')
        final_product = v.get('product')
        qty += v.get('quantity')

    V.append({'product': final_product, 'quantity': qty, 'warehouse': wa})
...
for i in V:
    ...
    if w not in dic:
        dic[w] = []
    dic[w].append({"product": i.get("product"), "quantity": i.get("quantity")})
print(dic)
