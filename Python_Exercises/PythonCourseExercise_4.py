mylist =[{'product': 'furn_0789', 'quantity': 16.0, 'warehouse': 'san francisco'},
...
final_dict = {}
# for i in mylist:
#     i.get('warehouse') not in final_dict.keys() and final_dict.update({i.get('warehouse') : []}) and final_dict.get(i.get('warehouse')).append({'product': i.get('product'), 'quantity': i.get('quantity')}) or final_dict.get(i.get('warehouse')).append({'product': i.get('product'), 'quantity': i.get('quantity')})
# # print(final_dict)

# def getResponse():
#     Response = input("Do you want to enter a record? (Yes/No):").lower()
#     if (Response == 'yes'):
...
for i in mylist:
    if (i.get('warehouse') not in final_dict):
        final_dict.update({i.get('warehouse'): []})
    final_dict.get(i.get('warehouse')).append({'product': i.get('product'), 'quantity': i.get('quantity')})
...
if (w not in final_dict.keys()):
    final_dict.update({w:[]})
    final_dict.get(w).append({'product': p, 'quantity': q})

else:
    for i in final_dict.get(w):
        if(p == i.get('product')):
            new_quantity = int(i.get('quantity')) + int(q)
            i.update({'quantity': new_quantity})

        if(i == len(final_dict.get(w))-1):
            final_dict.get(w).append({'product': p, 'quantity': q})
